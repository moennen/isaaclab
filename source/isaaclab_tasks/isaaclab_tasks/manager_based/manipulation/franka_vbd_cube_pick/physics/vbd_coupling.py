...
Imports
-------
Both ``vbd_newton_manager.py`` and standalone validation tools import from
...
def _kernel_body_particle_reaction(
    ...
    shape_material_ke: wp.array(dtype=wp.float32),
    ...
    Uses per-contact ``avg_ke = 0.5 * (soft_contact_ke + shape_material_ke[s])``
    to match VBD's own ``evaluate_body_particle_contact`` and restore Newton's
    third law symmetry.
    """
    tid = wp.tid()
    ...
    # Reaction stiffness: use soft_contact_ke (particle-side ke = 1000 N/m).
    # VBD internally uses avg_ke = 0.5*(soft_ke + shape_ke) ≈ 25500 N/m for
    # the VBD contact force on particles.  The intentional ke asymmetry
    # (reaction_ke = 1000 << VBD_ke = 25500) is REQUIRED for reliable lift:
    #   scale = mu * VBD_ke * δ_eq / eps_u ≈ 47800 N/m >> k_elastic ≈ 2000 N/m
    # → tracking efficiency η ≈ 0.96 → finger-cube lag < 5 mm over full 200 mm lift.
    # Symmetric ke would give scale ≈ 3750 N/m (ke_finger-limited, independent
    # of ke_avg), yielding η ≈ 0.65 and a 71 mm lag → contacts lost in 0.2 s.
    ke = soft_contact_ke

    # Force on particle (VBD spring: pushes particle away from surface).
    f_on_particle = n * (ke * penetration)
...
def _kernel_body_particle_reaction_with_friction(
    ...
    Friction model:
    - Per-contact ``avg_ke = 0.5 * (soft_contact_ke + shape_material_ke[s])``
      mirrors VBD's own stiffness averaging, restoring Newton's third law symmetry.
    ...
    # Reaction stiffness: use soft_contact_ke = 1000 N/m (asymmetric by design).
    # See _kernel_body_particle_reaction for detailed explanation.
    ke = soft_contact_ke

    normal_load   = ke * penetration
...
def apply_soft_body_reactions(
    ...
    *,
    ...
    The caller must call ``state.clear_forces()`` at the END of each substep
    (after the state swap, on the new state_0) so the next substep starts clean.
