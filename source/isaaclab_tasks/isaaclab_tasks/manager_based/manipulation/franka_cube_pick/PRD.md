...
- F13.7: `scripts/compute_observations.py` — extracts 25-dim obs from sequences file (no re-simulation needed: sequences already record joint_pos, joint_vel, ee_pos_w, cube_pos_w, gripper_closed).
- F13.8: `scripts/analyze_observations.py` — 7-analysis observation analyzer (same as rigid task but VBD paths; cube orientation excluded since deformable body has no unique orientation).
- F13.9: `scripts/visualize_sequence.py` — Newton ViewerGL + imgui sidebar; cube rendered as rigid box proxy driven by particle CoM.

### VBD Observation Space (25 dims)

  - `cube_pos` (3): cube XYZ in robot root frame = particle CoM [m]
  - `ee_pos` (3): EE position in robot root frame [m]
  - `joint_pos` (9): arm(7) + finger(2) [rad or m]
  - `joint_vel` (9): arm(7) + finger(2) velocities [rad/s or m/s]
  - `grip` (1): gripper closed flag (0.0 or 1.0)

Note: no cube_quat (deformable, no unique orientation), no ee_quat/linvel/angvel (not in scripted sequences).
...
- 2026-04-13: Validation scripts created (`scripts/`): generate_sequences.py, replay_sequences.py, analyze_results.py — follow same pattern as rigid skill 05.
- 2026-04-14: Identified and fixed GPU non-determinism bug: `_MAX_PARTICLE_VEL = 0.5 m/s` caps inertia displacement per substep. Non-deterministic VBD contact kernel atomic accumulation causes replay divergence at high velocities. Cap at 0.5 m/s: legitimate lift ≈0.27 m/s < cap; spurious spikes ≫ cap → clamped.
- 2026-04-14: Fixed cube resolution for grasping reliability. `cube_resolution=3→5`: 16 vs 36 surface particles/face; res=3 only 2–4 particles inside finger footprint (6/16 LIFTED=37%); res=5 always enough inside footprint (16/16=100%).
- 2026-04-14: Fixed ground contact instability. Particles tunneled 130mm below ground in one VBD step (FEM stiffness >> contact spring); exited `soft_contact_margin=27mm`; never re-detected. Fix: `soft_contact_margin = 180mm` (particle_radius×20), `shape_material_ke[ground]=1.0`, `shape_material_kd[ground]=2e-3`. Ground identified by `shape_body < 0` (body=-1), NOT collision_group (finalized model assigns cg=1 to ground plane). Side effect: cube CoM sinks 8mm to z≈0.026m — harmless, IK reads actual CoM.
- 2026-04-14: Generated 100-sequence validation dataset: `vbd_sequences_v3.json` (seed=42, 4 worlds). Label distribution: reachable_success=34, reachable_failure=37, unreachable_success=19, unreachable_failure=10. Computed observations: `vbd_observations.json` (25-dim, pure extraction, no re-simulation).
- 2026-04-14: Added `compute_observations.py` (pure Python extract, no simulation), `analyze_observations.py` (7 analyses), `visualize_sequence.py` (Newton ViewerGL). Added `compute_rewards_from_seqs.py` (fast reward evaluation from sequences state, no re-simulation).
- 2026-04-14: Full validation pipeline run (100 seqs, seed=42, vbd_sequences_v3.json):
  ACCURACY: 70% (70/100 OK). 36 TP, 34 TN, 17 FN, 13 FP.
  KEY ISSUE (now fixed): Approach collision — robot arm hits VBD cube during approach phase
  (t<2s), launching it to dist_xy=1.8m+ (outside r_max=0.65m). 91% of reachable frames
  have wrong reachable_mask → reward near zero → reachable branch R²=0.07.
  ROOT CAUSE: collision_group=2 on arm URDF shapes prevents rigid BOX-BOX contacts but
  does NOT affect VBD particle-rigid contacts, which use a separate COLLIDE_PARTICLES flag.
  Without clearing this flag, arm links collide with VBD particles during HOME→PRE_GRASP.
  Same root cause as rigid task (arm shapes in group=2 prevent cube contact), but for VBD
  the particle contact path requires the additional shape_flags step.
  GROUND CONTACT FIX CONFIRMED: unreachable sequences correctly stay at cube_z≈0.03m.
  Unreachable branch working well: unreachable_success mean=4187, R²=0.92.
  OBS ANALYSIS (vbd_sequences_v3 — stale, generated before arm-collision fix):
  25-dim, 14 PCs for 90% variance, near-Markovian (fv0 ΔR²=0.057).
  ee_z/j4 best predict signal rewards (r≈0.8). Cube_pos uncorrelated with pick rewards
  (cube flies off during approach).
- 2026-04-15: Fixed approach arm-collision for VBD particle contacts.
  In the rigid task, arm URDF shapes are assigned collision_group=2 → excluded from BOX-BOX
  contact list with the cube (group=1). For VBD particle-rigid contacts, collision_group is
  insufficient — the COLLIDE_PARTICLES shape flag must also be cleared.
  Fix applied in generate_sequences.py (after model.finalize()) and vbd_newton_manager.py
  (after model soft_contact params set, in start_simulation):
  ...
  Documented in skill 07 pitfall #8 and skill 07 section 9 (validation results).
  Regenerating sequences as vbd_sequences_v4.json (seed=42, same params as v3).
- 2026-04-15: v4 validation complete (100 seqs, seed=42, arm-collision fix applied):
  ACCURACY: 76% (76/100 OK). 36 TP, 40 TN, 17 FN, 7 FP.
  Reward means: reachable_success=134, reachable_failure=87, unreachable_success=4164, unreachable=0.08.
  OBS ANALYSIS (vbd_obs_report_v4/): reachable_success R²=0.094 (was 0.022 — 4.3× improvement),
  unreachable_success R²=0.975, ALL R²=0.778 (was 0.623). Reachable/Unreachable discriminability
  greatly improved: ee_z F=0.35, grip F=0.33 (was F≈0.05 with approach collision).
  Remaining limitation: VBD non-determinism → 50% physical lift rate for reachable_success seqs;
  reachable branch R² still low (0.094) because grasp outcome unpredictable from obs alone.
  Unreachable branch clean and predictable.
  Root cause of 50% failure rate identified (2026-04-15):
  - NOT a descent/approach geometry issue (6mm Y clearance confirmed, descent is clean)
  - CONTACT INSTABILITY during closing phase: ke/k_mu = 1e4/7143 = 1.40 > 1 → VBD diverges on
    first contact → cube launches vertically (3–12m) from Poisson expansion
  Fix: increase Young's modulus from 2e4 → 2e5 Pa (k_mu=71429, ke/k_mu=0.14 → stable)
  - 2e5 is rubber-like (~20% compression under grip), still clearly deformable
  - 2e7 also stable but VBD convergence fails (too stiff for 40 iterations)
  Test with 8 seqs (seed=42): 2e5 gives 2/2 reachable_success LIFTED, 0 phantom launches.
- 2026-04-15: Added CUDA graph to VBD generator (was missing from initial implementation;
  rigid task generator had it from the start). Same pattern: capture collide+simulate_two_phase
  (pure GPU); IK, control h2d writes, and state d2h reads remain outside graph.
  Confirmed captured successfully. Generating vbd_sequences_v5.json (100 seqs, seed=42,
  Young's=2e5, CUDA graph).
