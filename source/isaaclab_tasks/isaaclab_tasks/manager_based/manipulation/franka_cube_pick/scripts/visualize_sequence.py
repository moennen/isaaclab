...
Replays recorded sequences in the Newton interactive viewer with an
...
python visualize_sequence.py \\
    ...
    [--quiet]                  # suppress warp compile messages
...
--record flag
-------------
When --record is set, the visualizer writes one JSON file per sequence played
to <data_dir>/record_<seq_id>.json containing per-frame:
  - frame_idx, t
  - joint_pos            (from sequences.json)
  - ee_pos_fk            (computed by this visualizer's Newton FK)
  - ee_pos_json          (ee_pos_w stored by generator in sequences.json)
  - cube_pos_w           (from sequences.json)
  - gripper_closed       (from sequences.json)
  - rewards              (all terms, re-computed from FK EE position)
Use this to diagnose discrepancies between recorded and FK-computed state.
"""
...
# ---------------------------------------------------------------------------
# Panda URDF (same path as generator / replayer)
# ---------------------------------------------------------------------------
...
# Label display names for the filter combo
...
class SequenceVisualizer:
    ...
    def __init__(self, viewer, sequences: list, cfg: dict, model, hand_idx: int,
        ...
        self._record_frames: list = []   # accumulated per-frame dicts for current seq
        # Number of robot joint coords (arm+fingers); cube free-joint comes after
        self._n_robot_joints = 9

        # Build the ordered list of unique label strings present in the data
        seen = {}
        for s in sequences:
            key = label_description(s["label"])
            seen[key] = True
        self._unique_labels = list(seen.keys())  # insertion order = stable

        # Filter state: 0 = all, 1..N = specific label
        ...
        # Camera: slightly above and to the side of the robot
        viewer.set_model(model)
        # Side-front view: camera to the right of the robot, looking across the workspace.
        # Robot base at origin; workspace in +x [0, 0.8], floor at z=0.
        viewer.set_camera(
        ...
        # Register sidebar UI panel (ViewerNull in headless mode has no imgui)
    ...
    def _load_sequence(self, seq: dict):
        """Switch playback to a new sequence, resetting all playback state."""
        # Flush record for previous sequence before switching
        ...
        print(f"[visualize] {len(self.frames)} frames at 25 Hz  speed={self.speed}x\n")
        print(f"{'t':>6}  {'frame':>5}  {'total':>8}  {'approach':>8}  {'lift':>6}  "
              f"{'success':>8}  {'signal':>8}  {'mask':>4}")
        print("-" * 70)

    def _flush_record(self):
        """Write accumulated per-frame record for the current sequence to JSON."""
        seq_id = self.seq["id"]
    ...
    def _apply_filter(self):
        """Recompute the filtered list and reload to sequence 0 of the new list."""
    ...
    def _render_ui(self, imgui):
        """Sidebar panel: label filter + sequence list + speed slider."""
        ...
        if imgui.begin_list_box("##seqlist", list_height):
            for i, seq in enumerate(self._filtered):
                ...
                # Scroll to keep the selected item visible when filter changes
                if is_selected:
    ...
    def _apply_frame(self, idx: int):
        ...
        The cube free-joint coords drive the box geometry rendered by log_state.
        """
        ...
        joint_q_np[n:n+3] = cube_pos                   # tx, ty, tz
        joint_q_np[n+3:n+7] = [0.0, 0.0, 0.0, 1.0]    # qx, qy, qz, qw (identity)
    ...
    def _compute_reward(self, frame_idx: int) -> dict:
        ...
        # EE pos from FK (already computed in _apply_frame)
    ...
    def step(self):
        if self._paused if hasattr(self, "_paused") else False:
            return
        ...
        self.sim_time += (1.0 / 25.0) * self.speed
    ...
    def render(self):
        ...
        # Cube is now rendered as actual box geometry via log_state (see _apply_frame).
...
def main():
    ...
    parser.add_argument("--speed",       type=float, default=1.0,
                        help="Initial playback speed multiplier (1.0 = real-time at 25 Hz).")
    ...
    # Load ALL sequences — the sidebar widget handles selection
    ...
    # Build Newton model — robot + cube box so log_state renders the actual box geometry.
    # Cube is added as a free-floating body; _apply_frame sets its position each frame
    # via the free-joint coords so the box tracks the recorded cube_pos_w exactly.
    urdf_path = _find_panda_urdf()
    mb = newton.ModelBuilder()
    mb.add_ground_plane()
    mb.add_urdf(str(urdf_path), floating=False)
    hand_idx = next(i for i, l in enumerate(mb.body_label) if _PANDA_HAND_BODY_LABEL in l)
    ...
    record_dir = Path(args.input).parent if hasattr(args, "record") and args.record else None
