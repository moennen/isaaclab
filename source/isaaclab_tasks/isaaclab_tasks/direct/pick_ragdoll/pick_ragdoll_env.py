...
"""Pick-Ragdoll environment: Franka robot interacts with a deformable ragdoll using a coupled solver."""
...
from .pick_ragdoll_env_cfg import PickRagdollEnvCfg

logger = logging.getLogger(__name__)


class PickRagdollEnv(DirectRLEnv):
    cfg: PickRagdollEnvCfg

    def __init__(self, cfg: PickRagdollEnvCfg, render_mode: str | None = None, **kwargs):
        if cfg.control_mode == "velocity":
            for actuator in cfg.robot_cfg.actuators.values():
                actuator.stiffness = 0.0
                actuator.damping = 200.0
        
        cfg.robot_cfg.actuators["panda_hand"].stiffness = 1000.0
        ...
        logger.info(
            "PickRagdollEnv: control_mode=%s, action_scale=%s, interactive_ik=%s",
    ...
    def _setup_interactive_ik(self):
        ...
        try:
            ...
            if newton_model is None:
                logger.info("[PickRagdollEnv] Newton model not available; IK disabled.")
                return
            ...
            self._newton_viewer_gl = None
            logger.info("[PickRagdollEnv] Newton IK initialized (EE index=%d)", self._ee_ik_index)
            ...
            logger.info("[PickRagdollEnv] IK not available: %s", exc)
    ...
    def _apply_ik_action(self):
        ...
        if self._newton_viewer_gl is None:
            ...
            if self._newton_viewer_gl is not None:
                ...
                logger.info("[PickRagdollEnv] Newton viewer gizmo registered")
            else:
                logger.warning("[PickRagdollEnv] NewtonViewerGL not found in sim.visualizers")
    ...
    def _register_reset_key(self):
        ...
        def _on_key(symbol, modifiers, _self=self):
            if symbol == key.R:
                ...
                logger.info("[PickRagdollEnv] Reset requested via R key")
            elif symbol == key.G:
                _self._gripper_closed = not _self._gripper_closed
                logger.info("[PickRagdollEnv] Gripper %s via G key", "closed" if _self._gripper_closed else "open")

        self._newton_viewer_gl.renderer.register_key_press(_on_key)
        self._reset_key_registered = True
        logger.info("[PickRagdollEnv] R key (reset) and G key (gripper toggle) registered")

    def _setup_scene(self):
        self.robot = Articulation(self.cfg.robot_cfg)
        self.ragdoll = DeformableObject(self.cfg.ragdoll)
    ...
    def _get_observations(self) -> dict:
        self.ragdoll.update(self.step_dt)

        # Ragdoll centroid: mean of all nodal positions
        nodal_pos = wp.to_torch(self.ragdoll.data.nodal_pos_w)  # (num_envs, num_particles, 3)
        self._ragdoll_centroid = nodal_pos.mean(dim=1)  # (num_envs, 3)
        self._object_pos = self._ragdoll_centroid

        obs = torch.cat(
            (
                self.joint_pos[:, self._arm_joint_idx],  # (num_envs, 7)
                self.joint_vel[:, self._arm_joint_idx],  # (num_envs, 7)
                self._ragdoll_centroid,                   # (num_envs, 3)
            ),
            dim=-1,
        )
        return {"policy": obs}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ragdoll_height = self._ragdoll_centroid[:, 2]
        rew_ragdoll_height = self.cfg.rew_scale_ragdoll_height * ragdoll_height

        ee_pos = wp.to_torch(self.robot.data.body_pos_w)[:, self._ee_body_idx]
        ee_ragdoll_dist = torch.norm(ee_pos - self._ragdoll_centroid, dim=-1)
        rew_ee_ragdoll_dist = self.cfg.rew_scale_ee_ragdoll_dist * ee_ragdoll_dist

        rew_joint_vel = self.cfg.rew_scale_joint_vel * torch.sum(
            torch.abs(self.joint_vel[:, self._arm_joint_idx]), dim=-1
        )
        return rew_ragdoll_height + rew_ee_ragdoll_dist + rew_joint_vel
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # Reset deformable ragdoll to initial nodal positions
        env_ids_list = env_ids.cpu().tolist() if hasattr(env_ids, "cpu") else list(env_ids)
        self.ragdoll.reset(env_ids=env_ids_list)
